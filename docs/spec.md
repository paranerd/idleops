...
- **UI: der „Investor-Hub" (Entscheidung 12.07.2026)**: Es gibt **kein eigenes Finanzierungs-Panel** mehr. Das (i)-Popover neben der Bewertung ist der komplette Investor-Bereich — ganz oben ein **Fortschrittsbalken zur nächsten Sprosse** (siehe unten), darunter das Term Sheet (Aufschlüsselung inkl. × Dein Anteil = Verkaufserlös), der **Verkaufen-Button** und die **Finanzierungsrunden** (kompakte Zeilen mit „annehmen" und „aus eigener Tasche"). Ein pulsierendes **Badge** am (i) signalisiert ein wartendes Term Sheet; zusätzlich ein Toast. Das Popover ist rechtsbündig und in der Höhe gedeckelt (scrollt).
- **UI: Fortschritt zur nächsten Runde im Popover (Entscheidung 13.07.2026)**: Der frühere Header-Zusatz „· Series A ab 10 M" ist raus — der Header zeigt nur noch Münze + Bewertungswert. Stattdessen oben im Popover eine Zeile + Gold-Balken: „Nächste Runde: Series A ab 🪙 10 M · noch 🪙 7 M" (Balken misst den Bewertungs-Hochwasserstand gegen die Schwelle). Sonderfälle: Angebot liegt vor („annehmen unten"), Track-Record fehlt, alle Runden durch → Ziel Unicorn, Unicorn erreicht.
- **UI: Verkaufs-Flow ohne Vorab-Bestätigung, dafür rücknehmbar (Entscheidung 13.07.2026)**: „Verkaufen" ist **ein Klick** (die frühere „Wirklich verkaufen?"-Zwei-Klick-Bestätigung ist raus). Der Verkauf ist zunächst nur **vorläufig**: Er füllt Bank/Exit-Zähler und öffnet den Perk-Shop, lässt den laufenden Run aber unberührt. Erst **„Neu gründen"** macht ihn endgültig (Run-Reset). Im Perk-Overlay gibt es daneben **„Doch nicht verkaufen"** — das setzt den Meta-Stand exakt auf vor dem Verkauf zurück (inkl. etwaiger Perk-Käufe, via `structuredClone`-Snapshot) und kehrt in den laufenden Run zurück. So ersetzt die Rücknahme-Möglichkeit die alte Vorab-Bestätigung.
- **UI: Perk-Shop-Overlay**: Nach dem (vorläufigen) Verkauf öffnet ein Overlay den Perk-Shop; die Aktionsleiste („Doch nicht verkaufen" + „Neu gründen") klebt **sticky** unten und ist immer erreichbar — auch wenn kein Perk leistbar ist (dann Hinweis „Erlös bleibt in der Bank" + Button „Ohne Kauf neu gründen").
...
- **Der Exit läuft über den Investor-Hub** (Stand 13.07.2026): Das Term Sheet lebt im Bewertungs-Popover (alle Posten als exakte Rohwerte, dann × Dein Anteil = Erlös); „Verkaufen" (ein Klick) öffnet den Perk-Shop-Overlay. Der Verkauf ist bis „Neu gründen" rücknehmbar („Doch nicht verkaufen"). Der Run-Save und der Meta-Save (Perks, Bank, Exit-Zähler, Best-Bewertung) liegen in **getrennten localStorage-Keys** – der Run-Key ist unverändert (Kompatibilität mit bestehenden Spielständen).
